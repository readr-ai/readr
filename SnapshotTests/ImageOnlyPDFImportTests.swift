...
final class ImageOnlyPDFImportTests: XCTestCase {
    ...
    func testScannedPDFImportsWithOnePageChapterPerPageAndTheImageOnlyFlag() async throws {
        ...
        // Before ingest retains the source, and if that copy ever fails, the
        // metadata verdict must still keep PDF-only/frontier-free UI semantics.
        let model = AppModel(store: InMemoryLibraryStore(), seedsSampleBook: false)
        XCTAssertTrue(model.isPDF(book))
    }
    ...
    private func renderReader(for book: Book, at url: URL, name: String) -> NSBitmapImageRep? {
        ...
        let view = PDFReaderView(
            ...
            annotationActions: .constant(nil), wideChrome: true
        )
